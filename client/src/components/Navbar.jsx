...
const Navbar = () => {
  const { thisPage, userInfo } = useContext(AuthContext);
  return (
    <div>
      ...
      ) : userInfo && userInfo.role === "user" && thisPage !== "/"  ? (
        <>
          <nav className="bg-gradient-to-r from-primaryUser to-secondUser border-gray-200 dark:bg-gray-900">
      ...
      ) : userInfo && userInfo.role === "admin" && thisPage !== "/" ? (
        <>
          <nav className="bg-gradient-to-r from-primaryBusiness to-secondBusiness border-gray-200 dark:bg-gray-900">
            <div className="max-w-screen-xl flex items-center justify-between mx-auto p-2">
              <Modal name="Get-Started" />
              <ModalSelectRoles name="Modal-SelectRoles" />
              <Link
